        ...
        <CardContent className="pt-6">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex items-start gap-4">
              <div className={`h-14 w-14 rounded-2xl flex items-center justify-center ds-title-cards shrink-0 ${mentorProfile?.avatarClassName ?? "bg-primary text-primary-foreground"}`}>
                {mentorProfile?.initials ?? "M"}
              </div>
              <div className="space-y-2">
                <div>
                  <h1 className="ds-title-lg tracking-tight">{mentorProfile?.fullName ?? "Mentor Dashboard"}</h1>
                  <p className="ds-body text-muted-foreground mt-1">{mentorProfile?.bio || "Overview of your students and their projects."}</p>
                </div>
                <div className="flex flex-wrap gap-2">
                  {mentorProfile?.degree ? <Badge className={`${mentorProfile.badgeClassName} border-0`}>{mentorProfile.degree}</Badge> : null}
